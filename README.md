...
This project is **Vercel-ready**. TanStack Start is built through Nitro's
Vercel preset, so Vercel receives the correct server + static output instead
of a plain static SPA folder.
...
3. Vercel uses:
   ...
   - Output generated by Nitro for Vercel
4. Click **Deploy**.
...
- Do **not** set Vercel output directory to `dist/client`; that causes a
  static-only deployment and can return `404: NOT_FOUND` for this TanStack
  Start app.
...
### Other hosts
For non-Vercel hosts, use the matching TanStack Start/Nitro adapter for that
platform.
